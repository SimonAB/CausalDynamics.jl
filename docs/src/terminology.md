...
| **`relation_kind`** | Declared edge semantics (`:causal_influence`, `:constitutive_persistence`, `:constitutive_dependence`, `:participation`, …) |
| **`information_set` / availability** | Policy ``ℋ_t`` and `ObservationBridge.availability`: when a quantity may be used at decision time |
...
**Governing rule:** node multiplicity follows `temporal_support` and
...
or `FromOnsetSupport` and does **not** set `ontological_character`. Prefer
`temporal_support` as the source of truth for unrolling.
