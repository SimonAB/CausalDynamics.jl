...
@testset "panel bridge (identification → wide columns)" begin
    ...
    cols = temporal_adjustment_columns(result, u)
    ...
    adj = adjustment_columns(u, query)
    @test adj == temporal_adjustment_columns(result, u)
...
@testset "panel bridge maps single-node supports" begin
    spec = TemporalDAGSpec(
        nodes = [
