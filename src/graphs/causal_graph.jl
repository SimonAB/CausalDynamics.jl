...
Directed graph with optional node / edge / graph properties (names, attached data).

Wraps `DiGraph` and delegates Graphs.jl operations. Prefer `SimpleDiGraph` for
identification when you do not need properties; use `CausalGraph` when integrating
with TMLE / PPL façades that attach metadata.
...
See also: `set_node_prop!`, `get_node_prop`, `attach_data!`.
"""
